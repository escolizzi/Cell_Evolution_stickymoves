...
if len(sys.argv) <3:
  ...
  print "Usage: ./plot_MSDcenter...blabla.py <figure name> <nr of tracks to plot> <filename(s)> "
  sys.exit(1)
else:
  figname=sys.argv[1]
...
for filename in sys.argv[3:]:
  ...
  cm_xpos=[np.mean(x) for x in xpos]
  cm_ypos=[np.mean(x) for x in xpos]
  ...
  max_interval=timepoints[-1]/(2*timeresolution)
  ...
  for timeinterval in timepoints[min_interval: max_interval]:
      ...
      for i in range( 0,len_xpos, stepsize ):
          try:
              a=cm_xpos[i+stepsize]
          except:
              #print "Exceeding stepsize: ", i+stepsize
              #print "for cell c:",c
              break
          if cm_xpos[i+stepsize]>-1:
              sq_displ = (cm_xpos[i+stepsize]-cm_xpos[i])**2. + (cm_ypos[i+stepsize]-cm_ypos[i])**2.
