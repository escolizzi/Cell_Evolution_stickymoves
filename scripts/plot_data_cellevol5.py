...
#       for line in fin:
        ...
        # print kc0,kcA,kcP,kcC
        contacts=line[31:]
        # print contacts
        # THIS BELOW FAILS WHEN CELLS ARE NOT IN CONTACT WITH MEDIUM
        # contacts=[] 
        # for x in line[::-1]:
        #   contacts.append(x)
        #   if x == '0':
        ...
        lj_thistime.append( [ kj0,kjA,kjP,kjC,kj0t,kjPt ] )
...
# at every time step lj is like this [[k0,kA,kP,kC],[k0,kA,kP,kC],...]
a = ax10.plot( ltime, [ np.mean( zip(*x)[4] ) for x in lj ], label="kj0t")
bla = []
for thistime_lj_toblpl in lj_toblpl:
    transp=zip(*thistime_lj_toblpl)
    bla.append(transp[4])
...
# at every time step lj is like this [[k0,kA,kP,kC],[k0,kA,kP,kC],...]
a = ax12.plot( ltime, [ np.mean( zip(*x)[5] ) for x in lj ], label="kjPt")
bla = []
for thistime_lj_toblpl in lj_toblpl:
    transp=zip(*thistime_lj_toblpl)
    bla.append(transp[5])
