...
for bla,filename in enumerate(sys.argv[4:]):
    ...
    if 'bc_g-4_50c' in filename: 
         #makes sure to print only one trajectory
         if not lgmin4:
           printthis=1
         else: 
           printthis=0
         lgmin4.append(av_dist)
         color=(0,151/255.,1.) #light blue
         
    elif 'bc_g0_50c' in filename:
         if not lg0:
           printthis=1
         else: 
           printthis=0
