...
if len(sys.argv) <5:
  print "This is the program 'plot_holes_....py'"
  print "Usage: ./plot_holes_....py <output figure name> <cell targetarea> <peak_row> <peak_col> <filename> <filename> <filename> ..."
  sys.exit(1)
else:
  figname=sys.argv[1]
  ...
  with open(filename,"r") as fin:
    for line in fin:
      ...
      if int(line[0])!=timepoint:
        lholes.append(lholes_thistime)
        ltime.append(int(line[0]))
        timepoint=int(line[0])
        lholes_thistime=[]
      lholes_thistime.append(int(line[2]))
  
  howmany_bins = 50
  bin_edges = np.linspace(0. , 50, howmany_bins)
  hist1, bin_edges1 = np.histogram( [y for x in lholes for y in x], bins=bin_edges, density=True) #indiv cells hist
  ax.plot( bin_edges1[:-1],hist1, label=filename.split('/')[-1] )
  # ax.plot(ltime[:-1], [np.mean(x) for x in lholes ])
ax.set_xlim(0,20)
plt.legend()
plt.show()
sys.exit(1)
